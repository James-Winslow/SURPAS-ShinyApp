...
record <- c("name", "favourite_pkg", "used_shiny", "r_num_years", "os_type")

# which fields are mandatory
mandatory <- c("name", "favourite_pkg")
...
### ### ### ###
# requires tidyverse(tidyr/dplyr)
# manipulates the file with CPT codes and their corresponding names
# makes new dataframe with a variable that lists the (Medium length) 
# name of a procedure with the corresponding CPT code

#  Now that I have exported this, we don't need it anymore.
# namescpt <- cptnames %>% 
#   mutate (cpt = paste0("(",CPT,")",collapse = NULL)) %>%
#   unite("procedure",c(Medium,cpt),sep="", remove=FALSE)
# 
# write.csv(namescpt,"./namescpt.csv", row.names = FALSE)

# within merge
# all.x = TRUE #
# big first specialtyCPT  merging into namescpt 

#  Now that I have exported this, we don't need it anymore.
# specCPT <- merge(specialtyCPT,namescpt,all.x = TRUE,all.y = FALSE,by.x = "CPT")
# specCPT <- specCPT[,1:4]
# write.csv(specCPT,"./specCPT.csv", row.names = FALSE)
...
### ### ### ### ### ### ### ### ### ### ### ###
  ...
  colnames(allP) <- c("Individual Patient Risk", "National Average Risk")
  rownames(allP) <- c("Death", "Any", "UnplReadm",
                      "Infections", "Bleeding", "Pulmonary", "Urinary",
                      "VTE", "Cardiac", "Renal", "Stroke", "NonhomeDis")
  

allP$`National Average Risk`<-round(as.numeric(allP$`National Average Risk`),2)
  

  allP$`Postoperative Complication`  <- rownames(allP)
  allP$National <- as.numeric(allP$`National Average Risk`)
  allP$Patient <- as.numeric(allP$`Individual Patient Risk`)
  ...
  column_order <- c("Postoperative Complication","Individual Patient Risk","National Average Risk")
